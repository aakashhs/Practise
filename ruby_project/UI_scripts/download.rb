class Download
	...
	def self.download_login(response, connection, username)
		...
		(return true, username) if response =~ /mail/
		system("mv #{username.gsub(' ','_')}_history.pdf /host_msuser1/Downloads")
		false
	end

	def self.download_challenge(response, connection, challenge_name, username)
		...
		(return true, challenge_name) if response =~ /mail/
		puts "#{challenge_name}"
