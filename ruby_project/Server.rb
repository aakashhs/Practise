...
while client = server.accept do
	...
	begin
		...
		if response == "GET / HTTP/1.1\r\n"
			Tasklist.load_task_list(client, connection, username)
			...
			Verify.validate(response, connection)
			...
			Pages.login_page(client)
		elsif response =~ /tasklist/ and response !~ /css/
			username = "Guest"
			Tasklist.load_task_list(client, connection, username)
		...
		elsif response =~ /contactus/ and response !~ /send/ and response !~ /png/			
		...
		if response =~ /page/ and response !~ /homepage/
			Tasklist.send_content(response, client, connection)
		end
		if response =~ /\.png/ || response =~ /\.js/ || response =~ /\.css/
			 Profile.read(client, response)
		end 
	client.close
